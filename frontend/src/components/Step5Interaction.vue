          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Quick chat version of Report Agent with 4 professional tools, has Polylop's full memory</div>
              </div>
...
const selectReportAgentChat = () => {
  ...
  // Restore Report Agent's chat history
...
const renderMarkdown = (content) => {
  ...
  // Clean <br> tags before and after block elements
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Clean <p><br> followed by block elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tag after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix discontinuous ordered list numbering: when single <ol> is separated by paragraph content, keep numbering incremented
...
/* Fix ordered list numbering - use CSS counter to maintain continuous numbering across multiple ol */
.message-text {
